...
#include <conio.h>
#include <stdlib.h>
int carPositionX = 300; // Car's initial X-axis position
// Function to draw the startup screen by graphics.h library
...
void drawCar(int carX, int carY) {
    ...
    bar(carX - 10, carY, carX + 10, carY + 30);//Draws a filled rectangle with its top-left corner at (x1, y1)
...
int main() {
    ...
    while (1) {
        ...
        if (obstacleY > 480) {
            obstacleY = 0;
            obstacleX = 220 + (rand() % 160);//xposition will remain within range 220 to 379
            score++;
        }
        if (obstacleY + 30 >= 400 && obstacleY <= 430 &&
            obstacleX + 10 >= carPositionX - 10 && //obstacle at left car at right check
            obstacleX - 10 <= carPositionX + 10) {//obstacle at right car at right left
            cleardevice();
        ...
        drawObstacle(obstacleX, obstacleY);
        drawTrack();
        if (kbhit()) {//isse ye hoga ke car ki position change kiye bagair bhi obstacle chalta rahe ga
            inputKey = getch();
